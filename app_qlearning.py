...
for episode in range(episodes):
    ...
    while not done and step < threshold:
        ...
        action = choose_action_greedy(state_Q)
        #action = action_num
...
################
        ...
        next_action = choose_max_value(next_state)
...
# Display the chart
plt.savefig('Piechart3_Qlearning.png')
plt.show()

#fig = plt.figure()
#plot = plt.matshow(data[0], fignum=0)
plt.plot(ep_steps)
plt.title('AC Forest Fire Qlearning (∈=0.1,α=0.5)')
plt.xlabel("Number of steps")
plt.ylabel("Number of episodes")
plt.savefig('Fig_Qlearning_3.png')
plt.show()
...
plt.title('AC Qlearning (∈=0.1,α=0.5)')
...
anim.save("Test3_Qlearning.gif", writer = PillowWriter(fps = 5))
